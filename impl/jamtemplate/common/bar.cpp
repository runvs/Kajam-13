    ...
    , m_shapeProgress { std::make_shared<jt::Shape>() }
{
    ...
    m_shapeProgress->makeRect(jt::Vector2f { m_width - 2, m_height - 2 }, textureManager);
    m_shapeProgress->setPosition(jt::Vector2f { 0 + 1, m_height - 1 });
...
void jt::Bar::doUpdate(float elapsed)
{
    ...
    if (m_horizontal) {
        ...
        m_shapeProgress->setPosition(jt::Vector2f { m_shapeFull->getPosition().x + 1,
            m_shapeFull->getPosition().y + (1 - value) * m_height + 1 });
    }
